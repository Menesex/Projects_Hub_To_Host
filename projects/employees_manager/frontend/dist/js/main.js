...
async function loadEmployees() {
    ...
    const url = `/api/employees?status=${encodeURIComponent(status)}&search=${encodeURIComponent(search)}&search_type=${encodeURIComponent(searchType)}`;
...
async function handleRetire(id) {
    ...
    try {
        const response = await fetch(`/api/employees/${id}/retire`, { method: 'PATCH' });
        if (!response.ok) {
            const data = await response.json().catch(() => ({}));
